...
export default class HomeController extends Controller {
  ...
  public async createLettcode(){
    ...
    const info = await ctx.service.lettcode.createLettcode(data);
    // const dianzanData = {
    //   id: info.insertId,
    //   like: 0 
    // }
  ...
  public async updateLettcode(){
    const { ctx } = this;
    const data = ctx.request.body;
    const info = await ctx.service.lettcode.updateLettcode(data);
    this.ctx.body = {
      code: 200,
      data: info
    }
  ...
  public async deleteLettcode(){
    const { ctx } = this;
    const data = ctx.request.body;
    const info = await ctx.service.lettcode.deleteLettcode(data);
    this.ctx.body = {
      code: 200,
      data: info
    }
